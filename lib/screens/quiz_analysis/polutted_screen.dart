...
import 'package:skin/screens/quiz_analysis/scale_screen.dart';
import 'package:skin/theme/app_colors.dart';


class PoluttedScreen extends StatefulWidget {
  const PoluttedScreen({super.key});

  @override
  State<PoluttedScreen> createState() => _PoluttedScreenState();
}

class _PoluttedScreenState extends State<PoluttedScreen> {
  final List polluteList=[
    'Yes',
    'Somewhat polluted',
    'No',
    'I\'m not sure',
  ...
  Widget build(BuildContext context) {
    return  ReusableForScreen(requiresWidget: Column(
      ...
      children: [
        const ReusableText(title: 'Is the air where you live polluted?',weight: FontWeight.w600,size: 18,color: AppColors.buttonColor,),
        const SizedBox(height: 10,),
        ... List.generate(polluteList.length, (index) =>  GestureDetector(
          onTap: (){
            setState(() {
              _currentIndex=index;
            });
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ReusableText(title: polluteList[index],weight: FontWeight.w600,size: 18,color:_currentIndex==index?AppColors.fullWhiteColor: AppColors.fullBlackColor,),
                const Spacer(),
                Container(
